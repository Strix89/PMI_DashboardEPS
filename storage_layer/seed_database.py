...
class MetricPatternGenerator:
    ...
    def generate_cpu_pattern(
        self, pattern: str, timestamp: datetime, asset_id: str
    ) -> float:
        """
        ...
        if pattern == "stable_with_io_spikes":
            ...
            if hour == 3 and 0 <= minute <= 30:  # I/O spike at 3 AM
                ...
                hour in [9, 13, 17] and minute < 15
            ):  # Brief spikes during business hours
                base_cpu = 35.0
            ...
            spike_factor = (
                hash(f"{asset_id}_{timestamp.hour}_{timestamp.minute // 5}") % 100
            )
            ...
            if 9 <= hour <= 18 and day_of_week < 5:
                ...
                activity_factor = (
                    hash(f"{asset_id}_{timestamp.hour}_{timestamp.minute // 10}") % 100
                )
    ...
    def generate_memory_pattern(
        ...
        if pattern == "stable_with_io_spikes":
            ...
            base_memory = 40.0
            volatility_factor = (
                hash(f"{asset_id}_mem_{timestamp.hour}_{timestamp.minute // 10}") % 100
            )
            ...
            if 9 <= hour <= 18 and day_of_week < 5:
                ...
                dev_factor = (
                    hash(f"{asset_id}_mem_{timestamp.hour}_{timestamp.minute // 15}")
                    % 100
                )
                if dev_factor < 25:
    ...
    def generate_io_pattern(
        ...
        if pattern == "stable_with_io_spikes":
            ...
            base_io = 10.0
            io_spike = (
                hash(f"{asset_id}_io_{timestamp.hour}_{timestamp.minute // 3}") % 100
            )
            ...
            if 9 <= hour <= 18 and day_of_week < 5:
                ...
                build_factor = (
                    hash(f"{asset_id}_io_{timestamp.hour}_{timestamp.minute // 20}")
                    % 100
                )
    ...
    def generate_metric_value(
        self, metric_name: str, pattern: str, timestamp: datetime, asset_id: str
    ) -> float:
        """
...
def generate_time_series_data(
    ...
    logger.info(
        f"Generating {total_metrics:,} metrics for {len(machine_assets)} machines over {days} days"
    )
    ...
    while current_time < end_time:
                ...
                if len(metrics_batch) >= batch_size:
                    try:
                        ...
                        if current_log_time - last_progress_log >= progress_interval:
                            ...
                            eta_seconds = (
                                (total_metrics - processed_count) / metrics_per_second
                                if metrics_per_second > 0
                                else 0
                            )
                            eta_str = (
                            ...
                            logger.info(
                                f"Progress: {progress_pct:.1f}% - Inserted {inserted:,} metrics "
                        ...
                        if progress_tracker:
                            ...
                            checkpoint_interval = (
                                300  # Default, could be made configurable
                            )
                            if (
                                current_log_time - last_checkpoint_save
                                >= checkpoint_interval
                            ):
                        ...
                        logger.error(
                            f"Failed to insert metrics batch at {current_time.isoformat()}: {e}"
                        )
        ...
        if current_time.minute == 0:
            ...
            logger.info(
                f"Time progress: {time_progress_pct:.1f}% - Processing {current_time.isoformat()} "
    ...
    if metrics_batch:
        try:
            ...
            logger.info(
                f"Final batch: Inserted {inserted:,} metrics in {batch_duration:.2f}s"
            )
        except Exception as e:
    ...
    # Final statistics
    generation_duration = time.time() - (
        progress_tracker["start_time"].timestamp() if progress_tracker else time.time()
    )
...
def generate_metrics_for_timerange(
    asset_id: str,
    pattern: str,
    start_time: datetime,
    end_time: datetime,
...
def generate_polling_history(
    ...
    logger.info(
        f"Generating {total_polls:,} polling records for {len(all_assets)} assets over {days} days"
    )
    logger.info(f"Time range: {start_time.isoformat()} to {end_time.isoformat()}")
    ...
    if progress_tracker and progress_tracker.get("current_polling_date"):
        ...
        logger.info(
            f"Resuming polling history generation from {resume_time.isoformat()}"
        )
    ...
    while current_time < end_time:
        for asset in all_assets:
            ...
            response_time = generate_response_time(
                asset, current_time, availability_status
            )
            ...
            if len(polling_batch) >= batch_size:
                try:
                    ...
                    if current_log_time - last_progress_log >= 30:  # Every 30 seconds
                        progress_pct = (
                            processed_count / (total_polls * 2)
                        ) * 100  # *2 for both metrics
                        records_per_second = (
                            batch_size / batch_duration if batch_duration > 0 else 0
                        )

                        logger.info(
                            f"Polling Progress: {progress_pct:.1f}% - Inserted {inserted:,} records "
                    ...
                    # Update progress tracker
                    if progress_tracker:
                        progress_tracker["completed_polling_operations"] = (
                            total_inserted
                        )
                    ...
                    logger.error(
                        f"Failed to insert polling batch at {current_time.isoformat()}: {e}"
                    )
                    if progress_tracker:
                        progress_tracker["current_polling_date"] = current_time
                    raise
        ...
        if current_time.minute == 0:
            ...
            logger.info(
                f"Polling Time progress: {time_progress_pct:.1f}% - Processing {current_time.isoformat()}"
            )
...
def determine_service_availability(
    ...
    Returns:
        Availability status: 1.0 (up), 0.0 (down)
    """
    ...
    if degradation_pattern == "memory_pressure":
        ...
        if memory_pressure_cycle >= 5:  # Days 5-6 of the week
            degradation_chance = (
                memory_pressure_cycle - 4
            ) * 0.4  # 40% on day 5, 80% on day 6
            if random.random() < degradation_chance:
                return 0.0  # Service down due to memory pressure

    # Simulate random service failures (99.5% uptime target)
    service_reliability = 0.995
    random_factor = (
        hash(f"{asset['asset_id']}_{timestamp.hour}_{timestamp.minute}")
        % 10000
        / 10000.0
    )

    if random_factor > service_reliability:
        return 0.0  # Service down

    # Service is running normally
    return 1.0


def determine_machine_availability(
    asset: Dict[str, Any],
    timestamp: datetime,
    pattern_generator: MetricPatternGenerator,
) -> float:
    ...
    if random_factor > machine_reliability:
        return 0.0  # Machine down

    # Check for pattern-specific failures
    if pattern == "memory_leak_pattern":
        # Higher chance of failure during memory pressure
        days_since_epoch = (timestamp - datetime(2024, 1, 26, tzinfo=UTC)).days
        memory_pressure = min(1.0, (days_since_epoch % 7) / 7.0)

        if (
            memory_pressure > 0.8 and random.random() < 0.01
        ):  # 1% chance during high memory pressure
            return 0.0

    elif pattern == "volatile_load":
        # Occasional failures due to load spikes
        spike_factor = (
            hash(f"{asset['asset_id']}_load_{timestamp.hour}_{timestamp.minute // 5}")
            % 100
        )
        if spike_factor < 2:  # 2% chance of load-related failure
            return 0.0
...
def generate_response_time(
    asset: Dict[str, Any], timestamp: datetime, availability_status: float
) -> float:
    """
...
def generate_backup_history(
    ...
    logger.info(
        f"Generating {total_backups:,} backup records for {len(machine_assets)} machines over {days} days"
    )
    logger.info(f"Time range: {start_time.isoformat()} to {end_time.isoformat()}")
    ...
    while current_date <= end_date:
            ...
            if len(backup_batch) >= batch_size:
                try:
                    ...
                    if current_log_time - last_progress_log >= 30:  # Every 30 seconds
                        ...
                        logger.info(
                            f"Backup Progress: {progress_pct:.1f}% - Inserted {inserted:,} records "
                            f"(Total: {total_inserted:,}) - Rate: {records_per_second:.0f}/sec"
                        )
                        last_progress_log = current_log_time
                    ...
                    if progress_tracker:
                        ...
                        progress_tracker["current_backup_date"] = datetime.combine(
                            current_date, datetime.min.time()
                        ).replace(tzinfo=UTC)

                except Exception as e:
                    logger.error(
                        f"Failed to insert backup batch for {current_date}: {e}"
                    )
                    if progress_tracker:
                        progress_tracker["current_backup_date"] = datetime.combine(
                            current_date, datetime.min.time()
        ...
        if current_date.day % 7 == 0:  # Every week
            ...
            time_progress_pct = (
                (days_progress / total_days) * 100 if total_days > 0 else 100
            )
            logger.info(
                f"Backup Time progress: {time_progress_pct:.1f}% - Processing {current_date}"
            )
...
def determine_backup_success(
    asset_def: Dict[str, Any], backup_time: datetime, base_success_rate: float
) -> bool:
    """
...
def simulate_service_states(
    storage_manager: StorageManager, start_time: datetime, end_time: datetime
) -> int:
    """
    ...
    while current_date <= end_date:
        current_datetime = datetime.combine(current_date, datetime.min.time()).replace(
            tzinfo=UTC
        )
        ...
        for asset_def in INFRASTRUCTURE_ASSETS:
            ...
            if random.random() < 0.01:
                ...
                logger.info(
                    f"Simulating host degradation for {asset_id} on {current_date}"
                )
            ...
            for service_def in asset_def.get("services", []):
                ...
                try:
                    ...
                    if (
                        ...
                        and "degradation_pattern" in service_def
                    ):
                        service_data["degradation_reason"] = service_def[
                            "degradation_pattern"
                        ]

                    # Add downtime schedule if applicable
                    if (
                        "downtime_schedule" in service_def
                        and service_def["downtime_schedule"]
                    ):
                        service_data["downtime_schedule"] = service_def[
                            "downtime_schedule"
                        ]
...
def determine_service_status(
    service_def: Dict[str, Any], current_time: datetime, host_status: str, asset_id: str
) -> str:
    """
    ...
    if host_status != "running":
        logger.debug(
            f"Service {service_def['service_id']} inheriting host status: {host_status}"
        )
        return host_status
    ...
    if downtime_schedule:
        ...
        if start_hour > end_hour:  # e.g., 23:00-01:00
            if current_hour >= start_hour or current_hour <= end_hour:
                logger.debug(
                    f"Service {service_def['service_id']} in scheduled downtime"
                )
                return "stopped"
        else:  # Normal downtime window
    ...
    if degradation_pattern == "memory_pressure":
        ...
        if memory_pressure_cycle >= 5:  # Days 5-6 of the week
            ...
            degradation_chance = (
                memory_pressure_cycle - 4
            ) * 0.3  # 30% on day 5, 60% on day 6
            if random.random() < degradation_chance:
                logger.debug(
                    f"Service {service_def['service_id']} degraded due to memory pressure"
                )
                return "degraded"
    ...
    if random.random() > service_reliability:
        ...
        logger.debug(
            f"Service {service_def['service_id']} experiencing random issue: {issue_type}"
        )
        return issue_type
...
def create_service_state_history(
    storage_manager: StorageManager, start_time: datetime, end_time: datetime
) -> int:
    """
            ...
            for service_def in asset_def.get("services", []):
                ...
                if should_generate_state_change(service_def, current_time):
                    ...
                    try:
                        ...
                        logger.error(
                            f"Failed to save service state change for {service_id}: {e}"
                        )

        current_time += timedelta(hours=4)

    logger.info(
        f"Service state history creation complete. State changes: {state_changes}"
    )
    return state_changes


def should_generate_state_change(
    service_def: Dict[str, Any], timestamp: datetime
) -> bool:
    """
...
def status_to_numeric(status: str) -> float:
    ...
    """
    status_map = {"running": 100.0, "degraded": 50.0, "stopped": 0.0, "unknown": 25.0}
    return status_map.get(status, 25.0)


def setup_logging(
    log_level: str = "INFO",
    log_file: str = "storage_layer/logs/seed_database.log",
    quiet: bool = False,
) -> logging.Logger:
    """
...
def log_execution_summary(
    ...
    logger.info(
        f"Average insertion rate: {total_inserted / execution_time.total_seconds():.0f} records/second"
    )
...
def save_progress_checkpoint(
    progress: Dict[str, Any], checkpoint_file: str = "seed_progress.json"
) -> None:
    """
...
def load_progress_checkpoint(
    checkpoint_file: str = "seed_progress.json",
    ...
    try:
        ...
        for key in datetime_fields:
            if key in checkpoint_data and isinstance(checkpoint_data[key], str):
                try:
                    checkpoint_data[key] = datetime.fromisoformat(
                        checkpoint_data[key].replace("Z", "+00:00")
                    )
                except ValueError:
                    try:
                        checkpoint_data[key] = datetime.fromisoformat(
                        ...
                        logger.warning(
                            f"Invalid datetime format in checkpoint for field '{key}'"
                        )
        ...
        logger.info(
            f"Progress: {checkpoint_data['completed_operations']}/{checkpoint_data['total_operations']}"
        )
...
def main():
    ...
    logger.info(
        f"Parameters: days={args.days}, batch_size={args.batch_size}, "
    ...
    if args.resume:
        ...
        if progress:
            logger.info(
                f"Resuming from checkpoint: {progress['phase']} "
    ...
    if not progress:
        ...
        if not args.metrics_only and not args.polling_only:
            ...
            total_ops += (
                len(INFRASTRUCTURE_ASSETS)
                + len(BACKUP_JOBS)
                + sum(len(asset.get("services", [])) for asset in INFRASTRUCTURE_ASSETS)
            )
        ...
        if not args.assets_only and not args.metrics_only and not args.polling_only:
            ...
            machine_count = len(
                [
                    ...
                    if a["asset_type"]
                    in ["vm", "container", "physical_host", "proxmox_node"]
                ]
            )
    ...
    try:
        ...
        if not args.skip_cleanup and (
            not args.resume or progress["phase"] == "initialization"
        ):
        ...
        if (
            ...
            and not args.polling_only
            and not args.backup_only
            and (
                not args.resume
                or progress["phase"] in ["initialization", "cleanup_complete"]
            )
        ):
        ...
        if (
            ...
            and (
                not args.resume
                or progress["phase"]
                in ["initialization", "cleanup_complete", "assets_complete"]
            )
        ):
        ...
        if (
            ...
            state_updates = simulate_service_states(
                storage_manager, start_time, end_time
            )
        ...
        if progress:
            save_progress_checkpoint(progress, args.checkpoint_file)
            logger.info(
                f"Progress saved to {args.checkpoint_file}. Use --resume to continue from checkpoint."
            )
        sys.exit(1)
