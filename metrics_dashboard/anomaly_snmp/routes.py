...
from .exceptions import (
    AnomalySNMPError, SessionNotFoundError, SessionCorruptedError,
    ParameterValidationError, ConfigurationError, SimulationError,
    SimulationDataExhaustedError, handle_exception, validate_parameters,
    create_error_response
)
from .logging_config import (
    get_logger, operation_context, performance_monitor, 
    error_handler, log_operation_start, log_operation_success, log_operation_error
)
...
def handle_route_errors(operation_name: str = None):
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                log_operation_start(logger, op_name, 
                ...
                log_operation_success(logger, op_name, duration,
                                    endpoint=request.endpoint,
                                    method=request.method)
    ...
    with operation_context(logger, 'configure_page_access') as ctx:
        ...
        try:
            if 'anomaly_snmp' in session and 'config' in session['anomaly_snmp']:
                ...
                logger.info(f"Configurazione esistente trovata: contamination={existing_config.get('contamination', 'N/A')}")
            else:
                ctx['has_existing_config'] = False
                logger.info("Nessuna configurazione esistente trovata")
            
            return render_template('anomaly_configure.html', 
...
def dashboard():
    ...
    with operation_context(logger, 'dashboard_page_access') as ctx:
        ...
        required_artifacts = ['baseline', 'test_set_seed']  # Cambiato da test_set a test_set_seed
        missing_artifacts = []
...
def train_model():
    ...
    try:
        ...
        if not request.json:
            return jsonify({
                'success': False,
                'error': 'Dati JSON mancanti nella richiesta'
            }), 400
        ...
        logger.info(f"Avvio training modello AnomalySNMP con parametri: contamination={contamination:.3f}, train_split={train_split:.2f}")
        ...
        logger.info(f"Training completato: {len(dataset_info['training_normal'])} training, {len(test_set)} test")
        ...
        session['anomaly_snmp'] = {
            ...
            'dataset_stats': {
                ...
                'test_set_total': len(dataset_info['test_normal']) + len(dataset_info['test_anomalies'])
            },
            'model_artifacts': {
        ...
        logger.info(f"Configurazione AnomalySNMP salvata in sessione per contamination={contamination:.3f}")
        ...
        return jsonify({
            'success': False,
            'error': f'Parametro mancante: {str(e)}'
        }), 400
...
def get_next_point(offset):
    """
    API per recuperare il prossimo punto dati nella simulazione
    Implementa iterazione sequenziale attraverso test_set con gestione stato
    ...
    try:
        ...
        if offset < 0:
            return jsonify({
                'success': False,
                'error': 'Offset deve essere un numero positivo'
            }), 400
        ...
        if 'anomaly_snmp' not in session:
            return jsonify({
                'success': False,
                'error': 'Sessione AnomalySNMP non trovata. Configurare prima il modello.'
            }), 404
        
        # Verifica model artifacts
        if 'model_artifacts' not in session['anomaly_snmp']:
            return jsonify({
                'success': False,
                'error': 'Model artifacts non trovati. Eseguire prima il training.'
            }), 404
        ...
        logger.info(f"Richiesta punto dati AnomalySNMP con offset={offset}, contamination={config.get('contamination', 'N/A')}")
        
        # Gestione stato simulazione in sessione
        if 'current_index' not in simulation_state:
        ...
        # OTTIMIZZAZIONE: Genera punti al volo usando seed e metadati
        dataset_stats = session['anomaly_snmp'].get('dataset_stats', {})
        test_set_total = dataset_stats.get('test_set_total', 0)
        test_set_seed = model_artifacts.get('test_set_seed', 1234)
        
        if offset >= test_set_total:
            return jsonify({
                'success': False,
                'error': 'Fine del test set raggiunta'
            }), 404
        
        # Genera il punto dati al volo usando il seed e l'offset
        data_point = generate_test_point_on_demand(
            offset, 
            config, 
            dataset_stats, 
            model_artifacts['baseline'],
            test_set_seed
        )
        
        # Aggiorna offset corrente in model_artifacts
        model_artifacts['current_offset'] = offset + 1
        
        # Aggiorna statistiche di simulazione
        if 'statistics' not in simulation_state:
            simulation_state['statistics'] = {'normal_count': 0, 'anomaly_count': 0}
        
        # Aggiorna contatori basati sull'etichetta del punto dati
        if data_point['real_label'] == 'Normal':
            simulation_state['statistics']['normal_count'] += 1
        elif data_point['real_label'] == 'Anomaly':
            simulation_state['statistics']['anomaly_count'] += 1
        
        session['anomaly_snmp']['model_artifacts'] = model_artifacts
        session['anomaly_snmp']['simulation_state'] = simulation_state
        
        # Calcola max_points basato sui metadati del test_set
        max_points = test_set_total
        ...
        response_data = {
            ...
            'max_points': test_set_total,  # Aggiungi il totale corretto
            'current_offset': offset,
            'simulation_state': {
                'current_index': offset,
                'is_running': simulation_state.get('is_running', True),
        ...
        if not has_more_data:
            logger.info("Fine del dataset raggiunta, simulazione completata")
            response_data['message'] = 'Fine del dataset raggiunta'
        
        return jsonify(response_data)
        
    except Exception as e:
        logger.error(f"Errore nel recupero punto dati AnomalySNMP: {str(e)}")
        return jsonify({
            'success': False,
...
def _generate_sequential_data_point(offset, config):
    ...
    timestamp = base_time + timedelta(seconds=offset * 2)  # Avanza di 2 secondi per punto
    ...
    if offset < normal_test_count:
        ...
        features = [
            round(random.uniform(0.3, 0.7), 3) for _ in range(8)
        ]
        ...
        if anomaly_index < anomaly_count:
            ...
            for i in range(8):
                if random.random() < 0.3:  # 30% chance di valore estremo
                    features.append(round(random.uniform(0.0, 0.2) if random.random() < 0.5 else random.uniform(0.8, 1.0), 3))
                else:
...
def validate_config():
    ...
    try:
        ...
        if not (0.01 <= contamination <= 0.15):
            errors.append(f'Contamination ({contamination:.3f}) deve essere tra 0.01 e 0.15')
        ...
        return jsonify({
            'success': False,
            'error': f'Errore di validazione: {str(e)}'
        }), 500
...
def get_config():
    ...
    try:
        if 'anomaly_snmp' not in session:
            return jsonify({
                'success': False,
                'error': 'Nessuna configurazione trovata'
            }), 404
        ...
        return jsonify({
            'success': False,
            'error': f'Errore nel recupero: {str(e)}'
        }), 500
...
def reset_simulation():
    ...
    try:
        if 'anomaly_snmp' not in session:
            return jsonify({
                'success': False,
                'error': 'Nessuna sessione AnomalySNMP trovata'
            }), 404
        ...
        return jsonify({
            'success': False,
            'error': f'Errore nel reset: {str(e)}'
        }), 500
...
def simulation_control():
    ...
    try:
        if 'anomaly_snmp' not in session:
            return jsonify({
                'success': False,
                'error': 'Sessione AnomalySNMP non trovata'
            }), 404
        ...
        if action == 'play':
            ...
            try:
                ...
                if offset >= 0:
                    simulation_state['current_index'] = offset
                    session['anomaly_snmp']['model_artifacts']['current_offset'] = offset
                    logger.info(f"Simulazione saltata all'offset {offset}")
                else:
                    return jsonify({
                        'success': False,
                        'error': 'Offset deve essere positivo'
                    }), 400
            except (ValueError, TypeError):
                return jsonify({
                    'success': False,
        ...
        return jsonify({
            'success': False,
            'error': f'Errore nel controllo: {str(e)}'
        }), 500
...
def get_simulation_status():
    ...
    try:
        ...
        progress_percentage = (current_offset / max_points) * 100 if max_points > 0 else 0
        ...
        status = {
            ...
            'statistics': {
                'total_points_processed': simulation_state.get('total_points_processed', current_offset),
        ...
        return jsonify({
            'success': False,
            'error': f'Errore nel recupero stato: {str(e)}'
        }), 500
...
def prepare_dataset_with_oversampling(contamination, train_split):
    ...
    logger.info(f"Dataset originale: {normal_original} normali, {anomaly_original} anomalie")
    ...
    target_normal_oversampled = int(anomaly_for_test * (1 - contamination) / contamination)
    ...
    logger.info(f"Dataset preparato: {len(training_normal)} training, {len(test_normal)} test normali, {len(test_anomalies)} test anomalie")
...
def train_isolation_forest_correct(dataset_info, contamination):
    ...
    logger.info(f"Baseline calcolata: μ={baseline['mu_normal']:.3f}, σ={baseline['sigma_normal']:.3f}")
...
def prepare_shuffled_test_set(dataset_info, model_artifacts):
    ...
    logger.info(f"Preparazione test set: {len(test_normal)} normali + {len(test_anomalies)} anomalie")
    ...
    for i, record in enumerate(combined_test_set):
        record['offset'] = i
        record['timestamp'] = base_time + timedelta(seconds=i * 2)  # Avanza di 2 secondi per punto
...
def simulate_if_score(features=None, baseline=None, is_normal=True):
    ...
    if is_normal:
        ...
        raw_score = np.random.normal(baseline['mu_normal'] - 0.1, baseline['sigma_normal'])
    else:
        # Anomalie: raw_score sopra la baseline
        raw_score = np.random.normal(baseline['mu_normal'] + 0.3, baseline['sigma_normal'] * 2)
...
def generate_test_point_on_demand(offset, config, dataset_stats, baseline, seed):
    ...
    # Combina e shuffla
    combined_indices = [(i, 'Normal') for i in normal_indices] + [(i, 'Anomaly') for i in anomaly_indices]
