...
def get_next_point(offset):
    ...
    try:
        ...
        if offset < 0:
            return (
                jsonify(
                    {"success": False, "error": "Offset deve essere un numero positivo"}
                ),
                400,
            )
        ...
        logger.info(
            f"Richiesta punto dati AnomalySNMP con offset={offset}, contamination={config.get('contamination', 'N/A')}"
        )
        ...
        if offset >= test_set_total:
            return (
                jsonify({"success": False, "error": "Fine del test set raggiunta"}),
                404,
            )

        # Genera il punto dati al volo usando il seed e l'offset
        data_point = generate_test_point_on_demand(
            offset, config, dataset_stats, model_artifacts["baseline"], test_set_seed
        )
...
@anomaly_snmp_bp.route('/api/reset_simulation', methods=['POST'])
@handle_route_errors('reset_simulation')
def reset_simulation():
    """
    API per resettare completamente la simulazione
    """
    try:
        if 'anomaly_snmp' not in session:
            return jsonify({
