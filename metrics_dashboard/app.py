...
def get_available_days():
    """API per ottenere il numero di giorni disponibili nel DB"""
    try:
        storage_manager = StorageManager(
            connection_string=DATABASE_CONFIG['connection_string'],
            database_name=DATABASE_CONFIG['database_name']
        ...
        # Accesso diretto alla collection metrics
        # Prima vediamo che collections esistono nel database
        collections = storage_manager.database.list_collection_names()
        logger.info(f"Collections disponibili nel database: {collections}")
        
        if 'metrics' not in collections:
            logger.warning("Collection 'metrics' non trovata nel database!")
            storage_manager.disconnect()
            return jsonify({'success': True, 'available_days': 0})
        
        metrics_collection = storage_manager.database['metrics']
        
        # Contiamo i documenti nella collection
        total_metrics = metrics_collection.count_documents({})
        logger.info(f"Totale documenti nella collection metrics: {total_metrics}")
        
        # Prima vediamo che metric_name esistono (sono dentro meta.metric_name)
        metric_names = list(metrics_collection.distinct("meta.metric_name"))
        logger.info(f"Metric names disponibili nella collection metrics: {metric_names}")
        
        # Cerchiamo specificatamente le metriche di availability
        availability_metrics = ['availability_status', 'service_status', 'availability', 'uptime', 'health']
        found_metrics = [name for name in metric_names if name in availability_metrics]
        logger.info(f"Metriche di availability trovate: {found_metrics}")
        
        # Usiamo la prima metrica trovata, preferendo availability_status
        metric_to_use = None
        if 'availability_status' in metric_names:
            metric_to_use = 'availability_status'
        elif 'service_status' in metric_names:
            metric_to_use = 'service_status'
        elif found_metrics:
            metric_to_use = found_metrics[0]
        else:
            # Se non troviamo metriche specifiche, proviamo con qualsiasi metrica
            metric_to_use = metric_names[0] if metric_names else None
        
        logger.info(f"Metrica selezionata per il conteggio giorni: {metric_to_use}")
        
        if not metric_to_use:
            logger.warning("Nessuna metrica trovata nella collection metrics")
            storage_manager.disconnect()
            return jsonify({'success': True, 'available_days': 0})
        
        # Query per contare i giorni distinti (usando meta.metric_name)
        pipeline = [
            {"$match": {"meta.metric_name": metric_to_use}},
            {"$group": {
                "_id": {
                    "$dateToString": {
                        "format": "%Y-%m-%d",
        ...
        total_days = result[0]['total_days'] if result else 0
        
        storage_manager.disconnect()
        logger.info(f"Giorni disponibili nel DB: {total_days}")
        return jsonify({'success': True, 'available_days': total_days})
        
    except Exception as e:
        logger.error(f"Errore nel recupero dei giorni disponibili: {e}")
        return jsonify({'success': False, 'error': str(e), 'available_days': 0})
