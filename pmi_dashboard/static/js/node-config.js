...
class NodeConfigManager {
    ...
    setupEventListeners() {
        ...
        const testConnectionBtn = document.getElementById('test-node-connection-btn');
        ...
        // Form validation will be set up when form is shown
    ...
    async handleFormSubmit(e) {
        ...
        try {
            ...
            // Always add new node (no editing functionality)
            const result = await proxmoxAPI.addNode(formData);

            if (result.success) {
                this.showNotification('Node successfully added!', 'success');
                this.hideNodeForm();
    ...
    async testConnection() {
        ...
        const formData = this.getFormData();
        const testBtn = document.getElementById('test-node-connection-btn');
        ...
        try {
            ...
            // Test the configuration without saving (always for new nodes)
    ...
    setButtonLoading(button, loading) {
        ...
        if (loading) {
            ...
            const icon = button.querySelector('i');
            if (icon) {
                if (button.id === 'test-node-connection-btn') {
